...
class AuthController {
  ...
  firebaseLogin = asyncHandler(async (req, res) => {
    const { firebaseToken, idToken, token } = req.body;
    const resolvedToken = firebaseToken || idToken || token;
    const result = await authService.loginWithFirebase(resolvedToken);
    return ApiResponse.success(res, result, "Login successful");
  });
