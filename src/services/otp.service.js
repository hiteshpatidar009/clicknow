...
class OtpService {
  constructor() {
    ...
    this.phoneProvider = env.OTP_PROVIDER_PHONE || "twilio";
    this.allowMockFallback = String(env.OTP_ALLOW_MOCK_FALLBACK) === "true";
    this.smtpTransporter = null;
    this.twilioClient = null;
    this.phoneResendWindowMs = parseNumber(process.env.OTP_RESEND_WINDOW_SECONDS, 60) * 1000;
  }
  ...
  async verifyOtp({ email, phone, otp, role = "client", provider }) {
    ...
    if (phone) {
      ...
      const record = await otpRepository.findByPhone(normalizedPhone);
      ...
      await otpRepository.deleteByPhone(normalizedPhone);
  ...
  async sendPhoneOtp({ phone, role, recaptchaToken, provider }) {
    ...
    const existing = await otpRepository.findByPhone(normalizedPhone);
    if (existing?.lastSentAt) {
      const lastSentAt = new Date(existing.lastSentAt).getTime();
      if (Date.now() - lastSentAt < this.phoneResendWindowMs) {
        throw new ValidationError(
    ...
    if (requestedProvider === "twilio") {
      await this.sendOtpByTwilio(normalizedPhone, otp);
      await otpRepository.upsertByPhone(normalizedPhone, {
        otp,
        provider: "twilio",
        expiresAt,
        lastSentAt: new Date(),
  ...
  async sendOtpByTwilio(phone, otp) {
    const client = this.getTwilioClient();
    const from = env.TWILIO_PHONE_NUMBER;
    const body = `Your ClickNow OTP is ${otp}. It will expire in ${env.OTP_EXPIRY_MINUTES || 10} minutes.`;
    ...
    try {
      ...
      Logger.info("Twilio OTP SMS sent", {
        to: phone,
        sid: message?.sid,
      ...
      throw new ServiceUnavailableError(
        "OTP SMS delivery failed via Twilio. Check credentials and sender number.",
      );
    }
  }
  ...
  async sendOtpBySendGrid(email, otp) {
    ...
    const { name: fromName, email: fromEmail } = parseFromAddress(
    ...
    if (!fromEmail) {
      throw new ServiceUnavailableError(
        "Valid sender is required. Set SENDGRID_FROM_EMAIL.",
      );
    }
    ...
    try {
      console.log("SendGrid API key loaded:", Boolean(apiKey));
      console.log("SendGrid sender:", env.SENDGRID_FROM_EMAIL);
      const { status, data } = await axios.post(
        apiUrl,
        {
