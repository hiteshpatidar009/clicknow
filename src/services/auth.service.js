...
import { userRepository, pendingRegistrationRepository } from "../repositories/index.js";
...
class AuthService {
  ...
  async register(userData) {
    ...
    const expiresAt = new Date(
      Date.now() + (env.OTP_EXPIRY_MINUTES || 10) * 60 * 1000,
    );

    await pendingRegistrationRepository.upsertByEmail(email, {
    ...
    Logger.logAuth("register_pending", email, true, { role });
  ...
  async verifyOtp({ email, phone, otp, role, provider }) {
    ...
    // Scenario C: Email OTP verified but user not found -> complete pending registration
    if (email) {
      ...
      const createdUser = await userRepository.create(userDataToSave);
